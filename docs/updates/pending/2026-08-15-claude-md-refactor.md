...
Structural changes:
- **Merged the two git-identity sections** (`## Git identity` + `## Git identity conflict with platform Stop-hook`) into one `## Git identity` section. Same rules, three paragraphs instead of five. The untested author-vs-committer-email idea is kept but compressed to one line and explicitly marked as untested.
