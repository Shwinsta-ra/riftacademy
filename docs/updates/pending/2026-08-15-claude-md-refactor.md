...
Two, both decided by Ashwin on 2026-08-15 during this session.

**1. Ignore the harness-assigned `claude/*` branch.** Remote/web Code sessions are handed an auto-generated `claude/*` branch cut from `main` and told to push only there. Both halves are wrong for this repo: `claude/*` is not an allowed source for *any* target in `enforce-branch-flow.yml`, so it fails `check-source-branch` every time, and `main` is the wrong base for anything but a hotfix. `CLAUDE.md` now carries standing permission to override that directive — create your own branch from latest `origin/integration` (or `origin/main` for a hotfix) with an allowed prefix.

This session hit both halves. PR #228 was opened from `claude/md-pointer-tw5fue` and failed `check-source-branch`; the replacement PR #229 was initially cut from `main`, which pulled six unrelated promotion merge commits into the diff (8 commits for a 3-file change) before being rebased onto `integration`. The prior `claude/tcgcsv-run-failure-otgpyh` branch is the same pattern, one round earlier. Rejected the alternative of adding `claude/*` to the CI allowlist: the prefix encodes *where a change may merge to*, and a `claude/*` branch carries no change-class information, so allowing it into `integration` only would make hotfixes impossible while allowing it everywhere would let any Claude branch reach `main` with CI unable to tell a hotfix from a refactor. Claude's authorship is already recorded in the PR footer; a `claude-authored` label was offered and declined as unnecessary.

**2. Claude now merges every PR up to and including `staging`.** `feature`/`fix`/`hotfix` → `integration`, `integration` → `beta`, and `beta`/`hotfix` → `staging`, once required checks pass. **`staging` → `main` remains Ashwin's exclusively, manual, and unchanged** — that is the one merge Claude never makes. Guardrail written alongside it: don't merge past an open question; if a PR raises a decision still waiting on Ashwin, leave it.

**This supersedes `ADMIN_THREAD_REMIT.md` §2's concurrency model, which is corrected in the same commit.** That section stated "Ashwin merges every PR himself; that's the actual checkpoint" — with merge authority delegated, that human checkpoint no longer exists below `main`. The gate is now the required status checks plus the open-decision rule. The consequence worth naming: anything that genuinely needs Ashwin's eyes before landing must be raised explicitly as a decision, because a merge no longer pauses for him.
