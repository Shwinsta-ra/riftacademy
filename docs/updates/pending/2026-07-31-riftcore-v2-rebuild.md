...
**Pre-merge amendments (added to the same PR, not a follow-up):**

Three changes landed on top of the original rebuild, all cheapest while `CARD_EFFECT_REGISTRY` is still empty and nothing consumes these types:

1. **`Predicate` / `Selector` / `EventPredicate` are now serializable DATA, not closures.** Phase 4 loads ability definitions from Supabase, and functions can't round-trip through a database — closures would have made abilities unpersistable. They're now discriminated unions evaluated in exactly one place, `src/lib/core/predicates.ts` (`evaluatePredicate`, `resolveSelector`, `matchesEvent`). Beyond serialization this makes abilities inspectable (RiftIQ can explain *why* a line is legal), diffable (RiftEngine can compare a reconstruction against the catalog), and coverage-visible (an unrepresentable card fails loudly instead of hiding behind a lambda). **The vocabulary is deliberately incomplete and additive** — when Phase 4 hits a card it can't express, extend the union and file a schema-gap fragment. There is deliberately **no escape hatch**; adding `{op:"custom"; fn:...}` would reintroduce exactly the problem this removes.
2. **`hasKeyword` now resolves printed ∪ granted, minus Layer-2 removals** (CR 477.2/801.3). The legacy version read only the granted list, so printed keywords were invisible to it — Phase 2 diff §1 flagged this as a partial match. To support it without dragging the card catalog into the kernel, `GameObject` gained `printedKeywords` and `domains`, mirrored from the catalog at instantiation the same way `printedMight` already was. **The kernel remains catalog-free** (verified: no kernel module imports `cards.json`).
3. **Constructed deck size is codified as exactly 40 including the Chosen Champion** (TR 402.1/601.1.b, CR 103.2) — the legacy "champion is card 41, uncounted" framing is dropped. `FormatContext` gains `mainDeckMax`, and a new `validateDeck()` enforces size, the 3-copies-per-name limit, the 3-signature cap matching the Legend's champion tag, exactly 12 runes, domain identity (**multi-domain cards require ALL their domains**), unique battlefield names, and per-format ban legality. **Sealed/draft construction is explicitly deferred** — that format is roughly three months out and will be rebuilt from TR 602.4 when it's live; in particular, whether sealed's 25-card minimum includes the champion is left unresolved rather than guessed.

**Contested audit (CR 190.3.d) — clean.** Verified that no `Predicate` or `Selector` member can test or select on Contested, that Contested is **not** an `ObjectStatus`, and that Contested plus the Showdown/Combat staging marks live only on `GameState` as kernel-internal bookkeeping. To keep it that way, `ObjectStatus` is now derived from a runtime `OBJECT_STATUSES` list with a regression test asserting Contested and staging never appear in it — every `ObjectStatus` is effect-reachable via `{op:"hasStatus"}`, so that list is the actual boundary.
