...
- **Section 6's Drive root path is wrong, and this fragment is the first thing to catch it.** Section 6 gives `/Users/ashwinsathe/My Google Drive/Personal/Games/Riftbound/RiftAcademy/`. The real path is `/Users/ashwinsathe/My Google Drive/Ashwin/Games/Riftbound/RiftAcademy/`: `Ashwin`, not `Personal`. Confirmed by Ashwin and verified by directory listing 2026-08-15. **Already applied directly to Section 6 on this branch, with Ashwin's explicit approval, as a deliberate one-time exception to the fragment-only rule in Section 1a.** Nothing for the reconciler to apply here; this entry is the record of why the master doc was touched by a fix PR. Both mount-alias warnings below were folded into the same line.
