...
**(a) B16's worked example was wrong, and Core has now ruled on it — new decision B17.** B16 writes the code as `op-019-024-metal-prizewall`. But the same decision rejects "treat OP as its own set" as "explicitly contradicted by the design requirement that OP is not a set code", and v3 §2, §5, §8.4 and §13 items 4/7 all say the same. This PR implemented `ogs-019-024-metal-prizewall` — the **mirrored base set** — and raised the conflict as a `decision_request`.

**Core ruled (A), the mirrored base set, on 2026-08-16, recorded as `RiftCore_Decisions.md` decision B17**, having independently re-verified all five cited sources against the live documents. B16's example is ruled a documentation slip, not a competing design. **B16 is left untouched** (append-only), so B16 and B17 disagree on the page by design and **B17 governs** — §3b now carries this as a ruling box citing B17 rather than as a flagged discrepancy, so a reader arriving from B16's example finds the resolution instead of re-deriving it. B17's scope is the prefix only; `promo_treatment`, the index and migration 020 were confirmed unaffected. `RiftCore_Context.md`'s promo-printing open item was updated to match.
