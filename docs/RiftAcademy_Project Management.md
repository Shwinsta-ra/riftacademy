...
**Last updated:** July 28, 2026 (morning reconciliation). Folded in the 2 fragments that were sitting in `docs/updates/pending/`: the RiftIQ Vendetta Bombs puzzle set (authored July 23; its cached bomb count was corrected 19→18 on July 25 after an Ocean Drake double-count, confirmed by RiftIQ — see Section 9) and the card-inventory re-merge that fixed 1v1-ban handling, Recruit token naming, and dual-domain parsing (PR #127, July 27). Section 2's card-count framing corrected to match — `cards.json` and the source inventory both read 928 now, since 1v1-banned cards are flagged rather than deleted. Section 6's competitive-bans standing rule updated to match; it still described the old remove-on-ban behavior this reconciliation retired. Both fragments folded in and deleted per the fragment lifecycle. **Section 0's RiftCoach/RiftIQ-Batch-1 items were not refreshed this pass — no new fragment evidence covered them; still reflect July 23 state, flagged inline.** **This doc is no longer edited directly by feature/fix PRs — see Section 1a and `docs/updates/TEMPLATE.md`.**
...
**Not fully refreshed since July 23 — Vendetta Pre-Rift Session 1 (Friday July 24) has already happened.** The July 28 reconciliation only had fragment evidence for the two new items below (RiftIQ Vendetta Bombs, card-inventory pipeline fixes); the RiftCoach/RiftIQ-Batch-1 paragraphs below are unchanged from July 23 and need a fresh check-in, not fact from this pass.

**RiftIQ Vendetta Bombs (themed puzzle set, separate from Batch 1/Batch 2):** 3 of 4 puzzles (V1, V2, V4) ready; **V3 is blocked** — its intended answer targets a companion unit given 4 Might, above Lacerate's 3-or-less threshold, so the correct answer fails the same gate as the wrong ones. Needs the companion's Might corrected and a fresh gate audit before V3 can ship. Also: the TickTick task backing this work still shows the pre-correction 11-bomb list — update it to 18 (see Section 9) or point it at `docs/riftiq/RiftIQ_Vendetta_Bombs_Batch.md` §1 instead of duplicating the data.
