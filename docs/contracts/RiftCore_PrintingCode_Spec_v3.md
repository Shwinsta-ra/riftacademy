...
> **Discrepancy on the record.** B16's illustrative example string reads `op-019-024-metal-prizewall`. That contradicts B16's own rejected-alternative paragraph in the same decision — *"treat OP as its own set … **Rejected**, explicitly contradicted by the design requirement that OP is not a set code"* — and contradicts §2, §5, §8.4 and §13 carry-forward items 4 and 7. Five sources against one example string, one of them the same decision's own reasoning, so this section implements the base-set-mirroring reading and the example is treated as a slip. **It is flagged rather than silently corrected**, on the same principle that keeps fixture 13 withdrawn-with-reason rather than deleted: had this been resolved quietly, the next reader of B16 would re-derive it the other way. If Core rules the other direction, the fixtures in §8 rows 23–25 and the slug rule above are what change; the column, the constraint and migration 020 are unaffected.
...
- **`RiftCore_Decisions.md` B16** — the design decision §3b implements, and the record of the two alternatives it rejected (extending `printing_variant`; treating OP as its own set). **§3b flags one internal inconsistency in B16 rather than silently resolving it** — see the ⚠ box in §3b. Not folded in here: B16 is Core's decision record, and this spec quoting it is not the same as amending it.
