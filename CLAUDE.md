...
- **Deleting a *local* branch ref: allowed without asking for `feature/*` and `fix/*` only; every other ref still needs Ashwin's go-ahead in the session** (narrowed 2026-08-16; was a blanket "ask every time" from 2026-08-15). The rule above governs *remote* branches, which you may delete once verified — `git push origin --delete` prompts and goes through. For local refs, `.claude/settings.json` now encodes the boundary directly, so the permission layer and this doc say the same thing rather than one promising what the other blocks:
  - **`allow`: `git branch -d`/`-D` on a branch whose name starts with `feature/` or `fix/`** — both lowercase, matching the prefixes `.github/workflows/enforce-branch-flow.yml` permits into `integration` (`^(feature/.*|fix/.*|hotfix/.*)$`, line 17). These run without a prompt. **The verification requirement below is not waived by this** — the allow removes the prompt, not the need for evidence.
  - **`deny`: `git branch -d`/`-D` on `main`, `staging`, `beta`, `integration`, or any `hotfix/*`.** Blocked outright, not merely asked about. `hotfix/*` is excluded by design, not oversight: it's the one prefix that can target `main` directly, so a stray delete there is the most expensive.
  - Anything else — a `claude/*` branch, a bare name, anything you didn't create — matches neither list and falls through to a normal prompt. Surface it to Ashwin rather than answering the prompt on autopilot.
  - **This exists to serve `Code/README-Code.md`'s "end session" step 3**, which deletes the just-closed feature's own branch as part of that one command. That is the scope: "end session" is the confirmation, not a blanket invitation to delete branches at will, and its auto-delete only ever targets the branch belonging to the feature being closed — never an environment branch, never a `hotfix/*`, never someone else's stale branch.
  - **Why this changed.** The 2026-08-15 rule was a blanket deny, and `Code/README-Code.md`'s "end session" step 3 was written to auto-delete a feature's branch with the command itself as the confirmation. Those two could not both be true — `git branch -d` was denied at the settings level regardless of what any prose said, so "end session" promised a step it could not perform. Ashwin's call, 2026-08-16: loosen the deny to exactly the two prefixes "end session" needs, and keep it closed everywhere else. The deny-list is narrowed here, not retired.
  - **Do not route around the deny rule on your own judgment.** `git update-ref -d refs/heads/<name>` deletes the same ref through plumbing and does not match the denied patterns. For the refs still denied above it is **not yours to reach for unprompted** — using it silently would make the guard decorative. Ask; if Ashwin authorises a one-off, say plainly which command you're using and why it differs from the denied one before running it.
  - **Verification is still required before any local delete, allowed prefixes included.** Run both checks from the remote-deletion rule above (`git branch -r --merged origin/staging` *and* `git cherry origin/staging origin/<branch>`), record the tip SHA, and cite the evidence in your response. `git push origin <sha>:refs/heads/<name>` restores a remote branch; a local ref deleted without its SHA written down first is simply gone. Not prompting is not the same as not checking.
  - **Report deletions at the moment your work is promoted, not later.** When your PR reaches `staging` and your local branch is therefore redundant, that is the point to say so — "PR #N is on `staging`; deleted local `fix/foo` (`<sha>`), restore with `git push origin <sha>:refs/heads/fix/foo`". The alternative is what happened on 2026-08-15: sixteen dead refs accumulated until a dedicated cleanup task was needed, and one of them (`fix/config-toml-seed-path`) had to be diffed against a merged twin before anyone could tell whether its work had landed at all.
